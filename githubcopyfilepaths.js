...
// ==/UserScript==
;(function () {
    'use strict' // Add CSS for the button
...
            background-color: #18762f !important;
        }
...
        }
    `
    document.head.appendChild(style) // Function to get all file paths

    function getAllFilePaths() {
        const filePaths = [] // Find all copilot-diff-entry elements with data-file-path attribute
    ...
    function insertCopyButton() {
        ...
        if (filterInput) {
            ...
            button.setAttribute(
                'style',
                'background-color: #347D39 !important; color: #ffffff !important; padding: 2px 14px; border-radius: 6px; cursor: pointer; margin-bottom: 16px; width: 100%; border: 1px solid rgba(27, 31, 36, 0.15) !important;'
            )
            button.onclick = copyPathsToClipboard // Insert button before the filter input's parent container
    ...
    setTimeout(insertCopyButton, 1000) // Watch for changes in case GitHub dynamically updates content
